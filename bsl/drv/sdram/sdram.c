...
void sdram_cover_data(uint8_t value)
{
...
void sdram_write_byte(uint32_t addr, uint8_t value)
{
    uint8_t *p_sdram = (uint8_t *)addr;
    *p_sdram = value;
}

void sdram_write_stream(uint32_t addr, uint8_t *data, uint32_t size)
{
    uint8_t *p_sdram = (uint8_t *)addr;
...
#warning "TES_SIZE must be less than or equal to 256"
#endif
    uint8_t *sdram_data = (uint8_t *)SDRAM_ADDR;

    sdram_cover_data(0x00);
    SCB_InvalidateDCache_by_Addr(sdram_data, TEST_SIZE);
    for (uint32_t i = 0; i < TEST_SIZE; i++)
    {
        if (sdram_data[i] != 0x00)
        {
            return 1;
        }
    }
    ...
    sdram_write_stream((uint32_t)sdram_data, test_data, TEST_SIZE);
    SCB_InvalidateDCache_by_Addr(sdram_data, TEST_SIZE);
    for (uint32_t i = 0; i < TEST_SIZE; i++)
    {
